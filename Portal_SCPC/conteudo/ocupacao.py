...
# Configura localidade para português
...
def obter_setores(engine):
    query = "SELECT DISTINCT cd_setor_atendimento, ds_setor_atendimento FROM UNIDADE_ATENDIMENTO_V WHERE ie_situacao='A' AND cd_classif_setor = 3 AND cd_setor_atendimento in (select cd_setor_atendimento from SETOR_ATENDIMENTO where ie_situacao = 'A') ORDER BY ds_setor_atendimento"
...
def obter_dados_paciente(engine, nr_atendimento):
    ...
    WHERE a.nr_atendimento = b.nr_atendimento AND a.nr_atendimento = :nr_atendimento
    """
    with engine.connect() as connection:
